...
export default function App() {
  ...
  // debounce the search box so typing doesn't refilter/re-render per keystroke
  useEffect(() => {
    const t = window.setTimeout(() => setSearchTerm(searchInput.trim()), 250);
  ...
  const [drawnSites, setDrawnSites] = useState<SiteMarker[]>([]);
  const [drawnSectors, setDrawnSectors] = useState<SectorShape[]>([]);
  const drawTokenRef = useRef(0);

  /* ---------- chunked (non-blocking) map computation with real progress ----------
   * Runs after "Calculate & Draw" (hasDrawn) and whenever data or styling changes.
   * Work is split into time-sliced chunks so the UI thread stays responsive and
   * the progress bar reflects actual computation instead of a fake timer. */
  useEffect(() => {
    if (!hasDrawn || !filteredData.length) {
    ...
    const CHUNK = Math.max(2000, Math.ceil(total / 50));
    const sitesOut: SiteMarker[] = [];
    const sectorsOut: SectorShape[] = [];
    ...
    const tick = () => {
      if (token !== drawTokenRef.current) return; // superseded by newer run
      const end = Math.min(idx + CHUNK, total);
      computeSites(rows.slice(idx, end), cfg, sitesOut, seen);
      computeSectors(rows.slice(idx, end), cfg, sectorsOut);
      idx = end;
      setDrawProgress(Math.round((idx / total) * 100));
      if (idx < total) window.setTimeout(tick, 0);
      else {
        setDrawnSites(sitesOut);
        setDrawnSectors(sectorsOut);
        setIsDrawing(false);
      }
    };
    tick();
  }, [hasDrawn, filteredData, resolveCfg]);
  ...
  const ingest = (hs: string[], rows: DataRow[], name: string) => {
    ...
    // Default popups to a handful of columns (not every column) — building popup
    // HTML for all headers on every row is a major lag source with big files.
    const defaultPopupCols = hs.slice(0, 6);
    setMapping({ ...EMPTY_MAPPING, ...guess, sitePopupCols: defaultPopupCols, sectorPopupCols: defaultPopupCols });
    setHasDrawn(false);
    setSearchInput("");
    setSearchTerm("");
    setFilterColumn("");
  ...
  const handleDraw = () => {
    ...
    // Triggers the chunked compute effect; progress is real work, not a fake timer.
    setHasDrawn(true);
  };
...
<div className="map-toolbar">
            ...
            <button
              ...
              className={cn("zoom-rect-btn", zoomRectActive && "active")}
            >
